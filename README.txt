还有一个补充请移步到csdn，层次softmax差不多，以后有时间在看，还有一些内部的hash细节，以及排序，读取文件等等，需要再看。
csdn链接：https://blog.csdn.net/weixin_40500427/article/details/116197951
...
// 首先需要知道模型结构，然后知道学的参数是什么，最后才能知道如何学习？模型结构其实包含三层：输入层，中间层，以及损失函数层映射层，所以严格意义上来说就一层模型结构，一个输入通过该层，得到一个输出，然后将该输出通过一个损失函数就得到了loss，具体模型结构如图二。以负采样的为例。
// syn0：词向量矩阵   就是初始化的词矩阵
// syn1：层次softmax的权重矩阵
// syn1neg：负采样的权重矩阵
